...
export default function App() {
  ...
  const createFerry = async () => {
    ...
    const f = { id, name: newFerryName.trim(), apiKey: newFerryKey.trim(), provider: provider.id, icon: randomIcon(), u: new Date().toISOString() };
  ...
  if (screen === "home") return (
    <div style={{ maxWidth: 520, margin: "0 auto", padding: "50px 16px" }}>
      <div style={{ textAlign: "center", marginBottom: 30 }}>
        <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-0.5px", marginBottom: 4 }}>{"\u{1F6A2}"} Bucklebury</div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {ferries.map(f => (
            <div key={f.id} onClick={() => enterFerry(f)}
              style={{ padding: "12px 16px", borderRadius: 8, border: "0.5px solid #ddd", cursor: "pointer", minWidth: 120, flex: "0 0 auto" }}
              onMouseEnter={e => e.currentTarget.style.background = "#f5f5f0"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              <div style={{ fontSize: 13, fontWeight: 600 }}>{f.icon || "\u{1F6A2}"} {f.name}</div>
            </div>
          ))}
        </div>
      </div>}
  ...
  if (screen === "createFerry") return (
    <div style={{ maxWidth: 420, margin: "0 auto", padding: "50px 16px" }}>
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        <div style={{ fontSize: 24, fontWeight: 700, letterSpacing: "-0.5px", marginBottom: 4 }}>{"\u{1F6A2}"} Create a ferry</div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => { setScreen("home"); setFerryError(""); setNewFerryName(""); setNewFerryKey(""); }}
